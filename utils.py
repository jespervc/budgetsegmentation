...
def norm_brain(X):
    ...
    for channel in range(Nchannels): 
        ...
        brain_norm = brain - np.mean(brain,axis = 0)
        brain_norm /= np.std(brain,axis = 0)#brain or brain_norm? axis or no axis?
...
def augmentation(X, y, do):
    ...
    if np.random.random_sample()>0.75:
        ...
        if do[0] == 1:   
            ...
            X = random_rotation(X,      #Input tensor. Must be 3D.
                                alpha,   #Rotation range, in degrees.
                                row_axis=0, col_axis=1, channel_axis=2, 
                                fill_mode='nearest', 
                                cval=0.0, interpolation_order=1)
            y = random_rotation(y,      #Input tensor. Must be 3D.
                                alpha,   #Rotation range, in degrees.
                                row_axis=0, col_axis=1, channel_axis=2, 
                                fill_mode='nearest', 
                                cval=0.0, interpolation_order=1)
        ...
        if do[1] == 1: 
            Tx, Ty = 10, 10
            X = random_shift(X,
                             Tx, Ty,  #Tx: Width shift range, as a float fraction of the width.
