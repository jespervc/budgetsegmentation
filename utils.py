...
def norm_brain(X):
    ...
    for channel in range(Nchannels): 
        im = X[:,:,channel]
        brain = im[im!=0] #brain img is nonzero pixels

        ########## YOUR CODE ############
        # apply normalization on the brain area        
        brain_norm = brain - np.mean(brain) # axis: The default is to compute the mean of the flattened array.
        brain_norm /= np.std(brain) 
...
def augmentation(X, y, do):
    ...
    if np.random.random_sample()>0.75:
        ...
        if do[0] == 1:   
            alpha = random.uniform(-45, 45) # rand() uniformly

        # implement TRANSLATION with random values Tx, Ty between 0 and the 10% of the image shape
        if do[1] == 1:
            temp = 0.1*X.shape[0], 0.1*X.shape[1]
            trans_max = np.floor(temp) # floors both values to be integers and  =< 10% of image 
            Tx = random.randint(0,  trans_max[0]) #X and Y same shape.
            Ty = random.randint(0,  trans_max[1])
        # implement SHEAR with random angle beta between [0-10) degrees
        if do[2] == 1: 
            beta = 10*random.random() #10*[0,1)

        # implement random ZOOM Zx, Zy between -20%, +20% along the 2 axis
        if do[3] == 1: 
            Zx = random.uniform(.8,1.2)
            Zy = random.uniform(.8,1.2)
